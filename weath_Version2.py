...
def plot_temperature_trend(df, city, highlight_extremes=True):
    ...
    # Plot temperature range (min to max)
...
def show_map(df):
    ...
    for (city, state, country, lat, lon), group in city_groups:
        ...
        # Customize marker color based on extreme days percentage
...
def display_city_report(report):
    ...
    # Display safety tips if there were extreme events
    if report['extreme_events']:
...
def main():
    ...
    # Custom CSS with white text and black tabs with white text
    st.markdown("""
    <style>
    /* Main text color (changed to white) */
    ...
    if df is not None and not df.empty:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["🌍 Global View", "📊 City Comparison", "📍 City Reports", "📥 Download Data"])
        ...
        with tab3:
            ...
            # Hourly data visualization for selected day
